...
class Purchases extends MY_Controller
{
    ...
    public function upload_purchase_file($invoice_no)
    {
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if (!$this->upload->do_upload('purchase_file')) {
                ...
                echo $this->db->update('hjms_receivings', $data, array('invoice_no' => $invoice_no));
            }
        } else {
            ...
            $this->load->view('templates/header', $data);
            $this->load->view('receivings/v_purchase_file', $data);
            $this->load->view('templates/footer');
        }
    }
            ...
            if (count($posting_type_code) !== 0) {
                ...
                elseif ($purchaseType == 'credit' && $register_mode == 'return') {
                    ...
                    foreach ($credit_purchase as $values) {
                        ...
                        $this->db->update('hjms_receivings', $data, array('invoice_no' => $values['invoice_no']));
    ...
    public function purchase()
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('receivings/v_purchases', $data);
        $this->load->view('templates/footer');
    }

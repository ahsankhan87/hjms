...
class M_receivings extends CI_Model{
    ...
    function get_receivings($receiving_id = FALSE, $from_date = null, $to_date=null,$type=null)
    {
        ...
        {
            $query = $this->db->get_where('hjms_receivings');
            return $query->result_array();
        }
        
       $query = $this->db->get_where('hjms_receivings',array('receiving_id'=>$receiving_id));
       return $query->result_array();
    }
    ...
    function get_selected_receivings($from_date = null, $to_date=null)
    {
        ...
        $query = $this->db->get_where('hjms_receivings as r');
        return $query->result_array();
    ...
    function get_creditPurchases($supplier_id)
    {
       ...
       $query = $this->db->get_where('hjms_receivings',array('account'=>'credit','register_mode'=>'receive',
    ...
    function updatePaidAmount($invoice_no,$data)
    {
       ...
       $this->db->update('hjms_receivings',$data,array('invoice_no'=>$invoice_no));
    ...
    function get_receiving_items($new_invoice_no)//for receipt
    {
       ...
       $this->db->join('hjms_receivings_items as B','A.receiving_id = B.receiving_id');
       $query = $this->db->get_where('hjms_receivings as A',array('A.invoice_no'=>$new_invoice_no));
       return $query->result_array();
    ...
    {   
        ...
        // $this->db->where('company_id', $_SESSION['company_id']);
        $query = $this->db->get('hjms_receivings',1);
    ...
    public function get_totalCostByReceivingID($invoice_no)
    {
       ...
       $query = $this->db->get_where('hjms_receivings_items',array('invoice_no'=>$invoice_no));
       $rows = $query->row();
    ...
    {   
        ...
        $query = $this->db->get('hjms_receivings');
        return $query->result_array();
    ...
    function get_receiving_items_only($invoice_no)//for receipt
    {
       ...
       $this->db->where(array('invoice_no'=>$invoice_no));
       $query = $this->db->get('hjms_receivings_items');
       return $query->result_array();
    ...
    {
        $this->db->where(array('receiving_invoice_no'=>$invoice_no));
   ...
   function delete($invoice_no)
    {
        $this->db->delete('hjms_receivings',array('invoice_no'=>$invoice_no));

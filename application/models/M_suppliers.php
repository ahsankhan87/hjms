...
class M_suppliers extends CI_Model{
    ...
    public function get_suppliers($id = FALSE, $limit = 1000000, $offset = 0)
    {
        if($id === FALSE)
        {
            ...
            $query = $this->db->get_where('hjms_supplier');
        ...
        $query = $this->db->get_where('hjms_supplier',$options);
        $data = $query->result_array();
        return $data;
    }
    ...
    public function get_supplierCurrency($id)
    {
        ...
        $query = $this->db->get_where('hjms_supplier c',array('c.id'=> $id,'c.status'=>'active','c.company_id'=> $_SESSION['company_id']));
        $data = $query->result_array();
        return $data;
    }
    ...
    {
        $this->db->select('ppt.*');
        $this->db->join('hjms_supplier sp','sp.posting_type_id=ppt.id');
    ...
    {
        $this->db->select('spt.*');
        $this->db->join('hjms_supplier s','s.sale_posting_type_id=spt.id');
    ...
    function getSupplierDropDown()
    {
        ...
        $query = $this->db->get_where('hjms_supplier',array('status'=>'active'));
    ...
    function addSupplier()
        {
                ...
                $this->db->insert('hjms_supplier', $data);
     ...
     function updateSupplier()
        {
            ...
            $this->db->update('hjms_supplier', $data, array('id'=>$_POST['id']));
    ...
    function delete($id)
    {
        $query = $this->db->delete('hjms_supplier',array('id'=>$id));
    }
    function deleteSupplier($id,$op_balance_dr,$op_balance_cr)
    {
       ...
       $query = $this->db->get_where('pos_receivings',array('supplier_id'=>$id));
       if($query->num_rows() > 0)//IF receivings EXIST THEN DELETE supplier receivings
       {
           ...
           foreach($receivings as $receiving_item)
           {
                ...
                $this->db->delete('acc_entries',array('invoice_no'=>$receiving_item['invoice_no']));
                $this->db->delete('acc_entry_items',array('invoice_no'=>$receiving_item['invoice_no']));
                ...
                $this->db->delete('pos_receivings_items',array('invoice_no'=>$receiving_item['invoice_no']));
                            /////////////////////////////
                            //WHEN supplier DELTED ITS receivings DELETED AND PRODUCT WILL BE REVERSED            
           }
           
           //DELETE ENTRIES BY supplier_ID NOT INVOICE NO.
           $this->db->delete('hjms_supplier_payments',array('supplier_id'=>$id));
           $this->db->delete('pos_receivings',array('supplier_id'=>$id));
          ////////////////
          ///////ACCOUNTS ENDS
       }
    ...
    function inactivate($id,$op_balance_dr,$op_balance_cr)
    {
       ...
       //OPENING BALANCE IN supplier ACCOUNT
        ...
        $query = $this->db->update('hjms_supplier',array('status'=>'inactive'),array('id'=>$id));
    ...
    {
        $query = $this->db->get_where('hjms_supplier_payments',array('entry_id'=>$entry_id,
        'company_id'=> $_SESSION['company_id']));
...
//                    }
        ...
        $query = $this->db->delete('hjms_supplier_payments',array('entry_id'=>$entry_id));
        
    }
    
    function delete_entry_by_invoice_no($invoice_no)
    {
        $query = $this->db->get_where('hjms_supplier_payments',array('invoice_no'=>$invoice_no,
        'company_id'=> $_SESSION['company_id']));
        ...
        $query = $this->db->delete('hjms_supplier_payments',array('invoice_no'=>$invoice_no));
        
    }
    
    function get_supplier_payment_history($supplier_id,$invoice_no){
        
        $options = array('supplier_id'=> $supplier_id,'invoice_no'=>$invoice_no);
        
        $query = $this->db->get_where('hjms_supplier_payment_history',$options);
        $data = $query->result_array();
        return $data;    
    }
    ...
    function delete_supplier_payment_history($supplier_id,$invoice_no)
    {
        ...
        $query = $this->db->delete('hjms_supplier_payment_history',$options);
        
    }
    
    function activate($id)
    {
        $query = $this->db->update('hjms_supplier',array('status'=>'active'),array('id'=>$id));
